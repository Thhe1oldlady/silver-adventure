...
class TestChurnService:
    ...
    def test_preprocess_data(self, churn_service):
        ...
        processed_data = churn_service._preprocess_data(
            sample_data, "test_model", fit_preprocessors=True
        )
        ...
        processed_data2 = churn_service._preprocess_data(
            sample_data, "test_model", fit_preprocessors=False
        )
    ...
    async def test_predict_churn(self, churn_service, sample_customer_data):
        ...
        features = {
            k: v
            for k, v in sample_customer_data.items()
            if k in churn_service.get_required_features()
        }

        result = await churn_service.predict_churn(
            customer_id=sample_customer_data["customer_id"], features=features
        )
    ...
    def test_generate_recommendations(self, churn_service, sample_customer_data):
        ...
        high_risk_recommendations = churn_service._generate_recommendations(
            "High", sample_customer_data
        )
        ...
        medium_risk_recommendations = churn_service._generate_recommendations(
            "Medium", sample_customer_data
        )
        ...
        low_risk_recommendations = churn_service._generate_recommendations(
            "Low", sample_customer_data
        )
    ...
    async def test_predict_with_missing_features(self, churn_service):
        ...
        result = await churn_service.predict_churn(
            customer_id="TEST_PARTIAL", features=partial_features
        )
